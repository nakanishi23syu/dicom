...
実在するPACS製品の画面キャプチャを参考に、ワークリスト（検査一覧）・タイムラインの構成を似せている
（詳細は下記「実製品を参考にした画面構成」参照）。
...
実際のPACS製品のタイムライン画面（モダリティごとの行×検査日を横に並べる構成）に合わせて、
...
検査一覧・タイムラインの画面構成は、実在するPACS製品の画面キャプチャを参考にしている。真似た主な点:
